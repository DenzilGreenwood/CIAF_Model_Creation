...
\setupdocument{CIAF Data Structures Technical Specification}{v1.0.0}
\setupheaderfooter{CIAF Data Structures Technical Specification}{v1.0.0}

% Document metadata
\title{CIAF Data Structures: Technical Specification for Lazy Capsule Materialization}
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\color{ciafblue}\textbf{CIAF Data Structures}}
\fancyhead[R]{\color{ciafgray}Technical Specification}
...
\hypersetup{
    ...
    filecolor=magenta,
    urlcolor=cyan,
    pdftitle={CIAF Data Structures Technical Specification},
    pdfauthor={Denzil James Greenwood},
    pdfsubject={AI Governance Data Structures},
    pdfkeywords={AI Governance, Data Structures, Lazy Materialization, Cryptographic Auditing}
}
...
\begin{tikzpicture}[
    ...
    data/.style={rectangle, draw=green!60, thick, rounded corners=3pt, minimum width=2cm, minimum height=0.8cm, text centered, font=\tiny, fill=green!10, align=center},
    process/.style={diamond, draw=orange!60, thick, minimum width=1.5cm, minimum height=0.8cm, text centered, font=\tiny, fill=orange!10, align=center},
    arrow/.style={->, thick, color=gray!70}
]
...
\node[data, left=of inference] (lightweight) {Lightweight\\ Receipt};
\node[data, below=of inference] (capsule) {Capsule\\ Header};

% Process nodes
\node[process, below=of dataset] (materialize) {Materialize};
\node[process, above=of capsule] (integrate) {Integrate};
...
\draw[arrow] (training) -- (deployment);
\draw[arrow] (deployment) -- (inference);
\draw[arrow] (inference) -- (lightweight);
\draw[arrow] (lightweight) -- (materialize);
\draw[arrow] (materialize) -- (inference);
\draw[arrow] (inference) -- (integrate);
\draw[arrow] (integrate) -- (capsule);

% Background grouping
\begin{scope}[on background layer]
\node[rectangle, draw=blue!30, thick, rounded corners=5pt, fit={(dataset) (model) (training)}, fill=blue!5, inner sep=8pt] {};
\node[rectangle, draw=purple!30, thick, rounded corners=5pt, fit={(deployment) (inference) (lightweight)}, fill=purple!5, inner sep=8pt] {};
\end{scope}
