...
\begin{itemize}
\item Storage reduction achieved through \textbf{lightweight receipts} ($\sim$500 bytes) \textbf{replacing immediate storage of complete evidence} ($\sim$50KB). The 85\% reduction comes from deferred evidence materialization, not from Merkle anchoring or WORM compression.
...
\item Low materialization rate (typically <5\%) reflects actual audit access patterns in enterprise environments. Organizations with higher audit rates would see proportionally reduced storage savings.
\end{itemize}
...
\begin{itemize}
\item \textbf{Storage Reduction:} Achieves 85\% reduction in annual storage requirements by storing lightweight receipts (500 bytes) instead of complete audit evidence packages (50 KB) during normal operations. Complete evidence is materialized on-demand when audits occur (typically <5\% of operations). The savings are proportional to the volume of operations and materialization rate.
...
\item \textbf{Computational Complexity:} Receipt generation complexity dominated by hash computation at $O(|D|)$ where $|D|$ is input data size, with highly efficient batch verification at $O(\log n)$ through Merkle tree structures. Merkle trees provide verification efficiency; the primary storage reduction comes from deferred materialization.
\end{itemize}
...
For enterprise deployments requiring regulatory compliance and audit trail immutability, LCM integrates with Write-Once-Read-Many (WORM) storage systems. WORM provides immutability guarantees for stored data; the 85\% storage reduction comes from storing lightweight receipts in WORM rather than complete evidence packages, not from WORM compression technology itself.
